...
class BatchNegativeSample(Configurable):
    ...
    def _score_unique_targets(model, slot, triples, unique_targets) -> torch.Tensor:
        ...
        if slot == S:
            all_scores = model.score_po(triples[:, P], triples[:, O], entity_emb, unique_targets)
        elif slot == P:
            all_scores = model.score_so(triples[:, S], triples[:, O], unique_targets)
        elif slot == O:
            all_scores = model.score_sp(triples[:, S], triples[:, P], entity_emb, unique_targets)
        else:
            raise NotImplementedError
        return all_scores
...
# Todo: Change back from SamuelSampler
class SamuelSampler(BatchNegativeSample):
    def __init__(
        self,
        config: Config,
        configuration_key: str,
...
class KgeUniformSamuelSampler(KgeSampler):
    ...
    def _sample_shared(
        ...
        # Todo: Change back from SamuelSampler
        if self.get_option("samples_within_batch"):
            return SamuelSampler(self.config,
                self.configuration_key,
                positive_triples,
                slot,
                len(unique_samples), #num_samples,
                torch.tensor(unique_samples, dtype=torch.long),

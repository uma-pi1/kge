...
class HyperBandSearchJob(AutoSearchJob):
    ...
    def init_search(self):
        ...
        # Create workers
        for i in range(self.config.get("hyperband_search.n_workers")):
            w = HyperBandWorker(
                sleep_interval=self.config.get("hyperband_search.sleep_interval"),
    ...
    def run(self):
        ...
        hpb = HyperBand(
            ...
            run_id=self.config.get("hyperband_search.run_id"),
            nameserver=self.config.get("hyperband_search.host"),
            min_budget=self.config.get("hyperband_search.min_budget"),
            max_budget=self.config.get("hyperband_search.max_budget")
        )
        # Run it
        res = hpb.run(n_iterations=self.config.get("hyperband_search.n_iter"),
                      min_n_workers=self.config.get("hyperband_search.n_workers"))
        ...
        print('Total budget corresponds to %.1f full function evaluations.' % (
                    sum([r.budget for r in res.get_all_runs()]) / self.config.get("hyperband_search.max_budget")))
...
class HyperBandWorker(Worker):
    ...
    def compute(self, config, budget, **kwargs):
        ...
        best = kge.job.search._run_train_job((
            ...
            trial_no,
            conf,
            self.job_config.get("hyperband_search.num_train_trials"),
